...
class TemperatureMachine(StateMachine):
    ...
    def updateLights(self):
        ...
        if (self.current_state.id == "heat"):
            if (temp < self.setPoint):
                redLight.blink(on_time=0.5,off_time=0.5,fade_in_time=0.2,fade_out_time=0.2)
            elif (temp >= self.setPoint):
                redLight.on()             
        elif (self.current_state.id == "cool"):
            if (temp > self.setPoint):
                 blueLight.blink(on_time=0.5,off_time=0.5,fade_in_time=0.2,fade_out_time=0.2)
            elif (temp <= self.setPoint):
                blueLight.on()
    ...
    def getFahrenheit(self):
        # Generally the floating point isn't used in these calcs or display calls. 
        # Also my system had been throwing a library error very inconsistently where an attempt to read the sensor would randomly not return properly, 
        #causing the display update to break. To fix this, I assume that typically
        #this random bad tick would not be during a significant temperature deviation, 
        #so I cache the temp and return the cached temperature if the pull fails. 
        try:
            t = thSensor.temperature
            self.cached_temp_f = floor(((9/5) * t) + 32)
        except: 
            if (DEBUG):
                print("Temperature sensor failed.")
        return self.cached_temp_f
    ...
    def manageMyDisplay(self):
        ...
        while not self.endDisplay:
            ...
            if(altCounter < 6):
                # I was annoyed that updates to the temperature weren't constantly visible in a consistent spot, so I adjusted the displays. 
                lcd_line_2 = f'SET: {self.setPoint}F NOW: {self.getFahrenheit()}F'
    
                altCounter = altCounter + 1
            else:
                suffix = ''
                if ((self.getFahrenheit() < self.setPoint) & (self.current_state.id == 'heat') | (self.getFahrenheit() > self.setPoint) & (self.current_state.id == 'cool')):
                        suffix = 'ING'
                lcd_line_2 =  f'SET: {self.setPoint}F {self.current_state.id.upper()}{suffix}'
